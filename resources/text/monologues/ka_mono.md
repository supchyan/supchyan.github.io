...
The cool stuff about my overlay, that when avatar look changes, it plays a completly unique animation, which takes attention of everyone. For example, KimiAvatars has Virgil's outfit from DMC series, so swapping to this outfit, avatar cuts the screen with judgement cut similar to DMC V effect. I'm pretty cool in shader coding, yes. B)

What about dev-stack I worked with, well... I used Godot with C# support + TwitchLib as SDK for Twitch written in C#. 

Why Godot? Because it has capabilities to render multiple windows with custom properties, such as transparent window background. For main overlay window this was a killer-feature, since OBS can handle transparent windows too. Thus, a customer was satisfied, me was satisfied as well.

Will I recommend to use Godot for such project in 2025...? Idk. This engine is decent, but it can't operate with GPU memory even today, so probably it will be better to work with something like Defold instead. Just for note: an empty scene in Godot reserves 60% of my GPU holy moly I will never use Godot once ever again.
